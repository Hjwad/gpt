...
"""
SOURCE_BUTTONS = InlineKeyboardMarkup([[InlineKeyboardButton('المصدر', callback_data='hurr')], [InlineKeyboardButton("الدعم", url=f"https://t.me/{SUPPORT_GRP}"), InlineKeyboardButton(text="عودة", callback_data="HELP_BACK")]])
        ...
        try:
            ...
            try:
                await msg.reply_photo(
                    photo=START_IMG, caption=f"حسب قاعدة البيانات الخاصة بي، لم تنضم بعد إلى [قناة التحديث]({link})، إذا كنت تريد استخدامي، انضم إلى [قناة التحديث]({link}) وابدأ من جديد!",
...
async def chatbotofd(client, message):
    ...
    if not is_vick:
        ...
        await message.reply_text(f"تم تعطيل الروبوت!")
    if is_vick:
        await message.reply_text(f"الروبوت معطل بالفعل")
